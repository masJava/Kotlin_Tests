...
class DetailsFragmentEspressoTest {
    ...
    fun fragment_testBundle() {
        val fragmentArgs = bundleOf("TOTAL_COUNT_EXTRA" to 404)
        val scenario = launchFragmentInContainer<DetailsFragment>(fragmentArgs)
        scenario.moveToState(Lifecycle.State.RESUMED)

        onView(withId(R.id.totalCountDetailsTextView))
    ...
    fun fragment_testSetCountMethod() {
        scenario.onFragment { fragment ->
            fragment.setCount(404)
        }

        val assertion = matches(withText(TEST_NUMBER_OF_RESULTS_FAKE_404))
        onView(withId(R.id.totalCountDetailsTextView)).check(assertion)
    }

    @Test
    fun fragment_testIncrementButton() {
        onView(withId(R.id.incrementButton)).perform(click())
        onView(withId(R.id.totalCountDetailsTextView)).check(
            matches(
                withText(
